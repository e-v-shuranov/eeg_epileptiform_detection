...
def convert_signals_half_banana(signals, Rawdata):
    ...
    # Origin
    # ['EEG Fp2-F8', 'EEG F8-T2', 'EEG T2-T4', 'EEG T4-T6', 'EEG T6-O2', 'EEG Fp1-F7', 'EEG F7-T1', 'EEG T1-T3',
    #  'EEG T3-T5', 'EEG T5-O1', 'EEG Fp2-F4', 'EEG F4-C4', 'EEG C4-P4', 'EEG P4-O2', 'EEG Fp1-F3', 'EEG F3-C3',
    #  'EEG C3-P3', 'EEG P3-O1', 'EEG Fz-Cz', 'EEG Cz-Pz', 'ECG EKG']
    # clinic_standart = ['EEG Fp1-F7', 'EEG F7-T1', 'EEG T1-T3', 'EEG T3-T5',
    #                    'EEG T5-O1', 'EEG Fp2-F8', 'EEG F8-T2', 'EEG T2-T4',
    #                    'EEG T4-T6', 'EEG T6-O2', 'EEG Fp1-F3', 'EEG F3-C3',
    #                    'EEG C3-P3', 'EEG P3-O1', 'EEG Fp2-F4', 'EEG F4-C4',
    #                    'EEG C4-P4', 'EEG P4-O2']
...
def readEDF(fileName):
    ...
    signal_duration = Rawdata.times[-1]

    # If the signal is too short, extend it by padding with zeros
    min_duration = 34  # Define a minimum duration (e.g., 10 seconds)

    if signal_duration < min_duration:
        print(f"Signal too short ({signal_duration:.2f} seconds), padding with zeros to {min_duration} seconds")
        # Pad the data with zeros to reach the minimum duration
        n_channels, n_samples = Rawdata.get_data().shape
        padding = np.zeros((n_channels, int(min_duration * Rawdata.info['sfreq']) - n_samples))
        extended_data = np.hstack((Rawdata.get_data(), padding))

        final_raw_data = mne.io.RawArray(extended_data, Rawdata.info.copy())
        Rawdata = final_raw_data
...
root = "/media/public/Datasets/sample_hospital/data_classes"

# eval_out_dir = os.path.join(root, "processed_data_10_09_2024")
eval_out_dir = "/media/public/Datasets/sample_hospital/processed/processed_Standard"
if not os.path.exists(eval_out_dir):
    os.makedirs(eval_out_dir)

BaseDirTrain = os.path.join(root, "Standard")
fs = 200
...
for file in test_files:
    os.system(f"cp \"{os.path.join(root, 'processed_data_10_09_2024', file)}\" {os.path.join(root, 'processed', 'processed_test_all_clinic')}")
